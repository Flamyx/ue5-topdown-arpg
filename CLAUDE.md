...
- **Activating abilities from Python in PIE:** `asc.try_activate_ability_by_class(load_blueprint_class(...))` returns False even for granted abilities. Use handles: `c = unreal.GameplayTagContainer(); c.import_text('(GameplayTags=((TagName="Abilities.Lightning.Electrocute")))')` → `asc.find_all_abilities_with_tags(c, True)` → `asc.try_activate_ability(handle)`. Cursor-driven abilities (TargetDataUnderMouse, e.g. Electrocute's `StoreHitResult` cancel) still fail: an unfocused PIE viewport has no cursor hit, it ticks at ~3 fps, and SlateInspector exposes no viewport widget to hover. Hand-test those. `StartPIE` needs `{"options":{"bSimulate":false,"playMode":"PlayMode_InViewPort","warmupSeconds":3}}`. The player's attribute set: `ObjectIterator(unreal.AuraAttributeSet)` filtered on `UEDPIE` + `AuraPlayerState`.
- **GE components (e.g. TargetTagsGameplayEffectComponent) aren't Python classes.** Get the subobject path from `cdo.get_editor_property("ge_components")[i].get_path_name()`, then `editor_toolset.toolsets.object.ObjectTools.set_properties` with camelCase JSON (`inheritableGrantedTagsContainer` → `added` + `combinedTags`). Modifiers: `BlueprintService.set_property(ge, "Modifiers", "(<export_text>)")`, with `CustomMagnitude=(CalculationClassMagnitude="/Script/Aura.MMC_X",Coefficient=(Value=..))`. Python drops the `b` prefix on bools (`bFooBar` → `foo_bar`).
- `AssetTools.create_asset(..., unreal.CurveTable, unreal.CurveTableFactory())` opens no dialog but adds a default row named `Curve`; remove it with `DataTableFunctionLibrary.remove_curve_table_row`. `get_curve_table_keys` only reads SimpleCurve tables (CT_Damage is RichCurve and returns `[]`; use `evaluate_curve_table_row`), and `SimpleCurveKey` has no `.time`; use `export_text()`.
- **Custom target-data ability tasks: copy the handle BEFORE `ConsumeClientReplicatedTargetData`.** When the client's sample was cached before the server task bound, `CallReplicatedTargetDataDelegatesIfSet` passes a reference INTO the ASC cache, and consuming clears it, so the broadcast carries an empty handle (a hit with `bBlockingHit=false`). Symptom (confirmed 2026-09-14): on a client-cast Electrocute the server's `MouseHitLocation` froze at the activation point, so no turning and no damage, while the log spammed `overriding pending replicated target data`. Also, the target-data delegate is keyed per ACTIVATION, not per task: a server task that never unbinds re-fires for every later sample of that activation (UTargetDataUnderMouse now unbinds + ends after one sample).
